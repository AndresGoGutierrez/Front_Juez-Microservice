...
const ProblemCreate = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await adminProblemService.create(formData)
      console.log("Problema creado:", response)

      // Redirigir a la página de detalles del problema
      navigate(`/admin/problems/${response.id_problem}`)
    } catch (err) {
      console.error("Error al crear problema:", err)
      setError(err.message || "Error al crear el problema. Por favor, intenta de nuevo.")
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <AdminLayout>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Crear Nuevo Problema</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {formData.test_cases.map((testCase, index) => (
            <div key={index} className="mb-6 p-4 border border-gray-200 rounded-lg">
              <div className="flex justify-between items-center mb-4">
                ...
                <button type="button" onClick={() => removeTestCase(index)} className="text-red-600 hover:text-red-800">
                  Eliminar
                </button>
              </div>
        ...
        <div className="flex justify-end space-x-3">
          ...
          <button
            type="submit"
            disabled={submitting}
            className="px-4 py-2 bg-primary-200 text-black rounded-md hover:bg-primary-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 disabled:opacity-50"
          >
